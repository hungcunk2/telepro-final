...
const ChatView: FC<ChatViewProps> = ({
  ...
  if (data?.empty)
    return (
      <div className=" bg-lighten flex-grow">
        <p className="mt-40 text-center text-gray-400">No message recently</p>
      </div>
    );
  ...
  return (
    <InfiniteScroll
      ...
      loader={
        <div className="bg-light flex justify-center py-3">
          <Spin />
        </div>
      }
      style={{
    ...
    >
      <div className="bg-lighten flex flex-col items-stretch gap-3 pt-10 pb-1">
        {data?.docs
          ...
          .map((item, index) => (
            <Fragment key={item.id}>
              ...
              {Object.entries(conversation.seen).filter(
                ...
                <div className="bg-lighten flex justify-end gap-[1px] px-8">
